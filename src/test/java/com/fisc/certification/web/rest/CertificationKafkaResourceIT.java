package com.fisc.certification.web.rest;

import com.fisc.certification.config.KafkaProperties;
...
class CertificationKafkaResourceIT {
    ...
    void setup() {
        ...
        CertificationKafkaResource kafkaResource = new CertificationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/certification-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/certification-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
